...
class TestPublishTrade:
    def test_publishes_to_raw_trades_topic(self, publisher: MarketDataPublisher, mock_producer: MagicMock):
    ...
    def test_uses_symbol_as_key(self, publisher: MarketDataPublisher, mock_producer: MagicMock):
    ...
    def test_value_is_json_string(self, publisher: MarketDataPublisher, mock_producer: MagicMock):
        trade = Trade(symbol="BTCUSDT", price=42000.0, quantity=0.1)
        publisher.publish(trade)
    ...
    def test_increments_trade_count(self, publisher: MarketDataPublisher):
        assert publisher.stats["trades_published"] == 0
        publisher.publish(Trade(symbol="BTCUSDT"))
        publisher.publish(Trade(symbol="BTCUSDT"))
        assert publisher.stats["trades_published"] == 2

    def test_calls_poll_after_produce(self, publisher: MarketDataPublisher, mock_producer: MagicMock):
        publisher.publish(Trade(symbol="BTCUSDT"))
        mock_producer.poll.assert_called_once_with(0.0)


class TestPublishDepthUpdate:
    def test_publishes_to_raw_depth_topic(self, publisher: MarketDataPublisher, mock_producer: MagicMock):
    ...
    def test_increments_depth_count(self, publisher: MarketDataPublisher):
...
class TestPublisherFlush:
    def test_flush_delegates_to_producer(self, publisher: MarketDataPublisher, mock_producer: MagicMock):
