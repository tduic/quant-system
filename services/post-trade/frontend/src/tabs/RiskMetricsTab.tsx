    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <Card
            ...
            positive={Number(data.sharpe_ratio) > 0}
          />
          <Card
            title="Sortino Ratio"
            value={fmt(data.sortino_ratio)}
            positive={Number(data.sortino_ratio) > 0}
          />
          <Card
            title="Calmar Ratio"
            value={fmt(data.calmar_ratio)}
            positive={Number(data.calmar_ratio) > 0}
          />
          <Card
            title="Profit Factor"
            value={fmt(data.profit_factor)}
            positive={Number(data.profit_factor) > 1}
          />
        </div>
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <Card
            title="Total Return"
            value={`${fmt(data.total_return_pct)}%`}
            positive={data.total_return_pct >= 0}
          />
          <Card
            title="Annualized Return"
            value={`${fmt(data.annualized_return_pct)}%`}
            positive={data.annualized_return_pct >= 0}
          />
          <Card
            title="Win Rate"
            value={`${fmt(data.win_rate_pct)}%`}
            positive={data.win_rate_pct > 50}
          />
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <Card
            ...
            value={`${data.max_drawdown_duration_periods} periods`}
          />
          <Card
