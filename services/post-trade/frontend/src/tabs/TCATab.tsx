    ...
    <div className="space-y-6">
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        <Card
          ...
          value={data.averages.spread_cost_bps.toFixed(2)}
        />
        <Card
          title="Avg Slippage (bps)"
          value={data.averages.slippage_bps.toFixed(2)}
        />
        <Card
          title="Avg Impact (bps)"
          value={data.averages.market_impact_bps.toFixed(2)}
        />
        <Card title="Avg Fee (bps)" value={data.averages.fee_bps.toFixed(2)} />
        <Card
          title="Avg Total Cost (bps)"
          value={data.averages.total_cost_bps.toFixed(2)}
        />
      </div>
                ...
                {data.fills.map((fill) => (
                  ...
                  >
                    ...
                    <td className="text-right py-2 px-3">
                      {fill.spread_cost_bps.toFixed(2)}
                    </td>
                    <td className="text-right py-2 px-3">
                      {fill.slippage_bps.toFixed(2)}
                    </td>
                    <td className="text-right py-2 px-3">
                      {fill.market_impact_bps.toFixed(2)}
                    </td>
                    <td className="text-right py-2 px-3">
                      {fill.fee_bps.toFixed(2)}
                    </td>
                    <td className="text-right py-2 px-3 font-medium">
                      {fill.total_cost_bps.toFixed(2)}
                    </td>
                  </tr>
                ))}
              </tbody>
