...
export function AlphaDecayTab({ symbol }: { symbol?: string }) {
  ...
  const hasData = data.total_signals > 0 && data.horizons.some((h) => h.ic !== null);
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Card
          ...
          value={
            data.horizons.some((h) => h.ic !== null)
              ? Math.max(...data.horizons.filter((h) => h.ic !== null).map((h) => h.ic!)).toFixed(4)
              : 'N/A'
          }
        ...
        <Card
          ...
          value={data.horizons.find((h) => h.filled_count > 0)?.horizon_label ?? 'N/A'}
        />
      </div>
      ...
      {!hasData ? (
        <div className="flex items-center justify-center py-16">
          <div className="text-center max-w-md">
            <h3 className="text-lg font-semibold text-gray-200 mb-2">Waiting for Signal Data</h3>
            <p className="text-gray-400 text-sm leading-relaxed">
              IC decay curves will appear once enough signals have been emitted and sufficient time
              has passed for horizon returns to be evaluated. At least 5 signals with filled
              horizons are needed.
            </p>
          </div>
        </div>
              ...
              <ResponsiveContainer width="100%" height={300}>
                <BarChart data={chartData}>
                  ...
                  <Tooltip
                    ...
                    formatter={(value: number) => [typeof value === 'number' ? value.toFixed(4) : 'N/A', 'IC']}
                  />
          ...
          <div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {data.horizons.map((h) => (
                    <tr key={h.horizon_ms} className="border-b border-gray-800/50">
                      <td className="py-2 px-3 font-medium">{h.horizon_label}</td>
                      <td className={`text-right py-2 px-3 font-mono ${
                        h.ic === null ? 'text-gray-500' : h.ic > 0 ? 'text-emerald-400' : 'text-red-400'
                      }`}>
                        {h.ic !== null ? h.ic.toFixed(4) : 'N/A'}
                      </td>
                      <td className="text-right py-2 px-3">{h.filled_count}</td>
                      <td className="text-right py-2 px-3">{h.total_signals}</td>
            ...
            <div>
              ...
              <div className="grid gap-4">
                {strategyIds.map((stratId) => {
                  ...
                  return (
                    <div key={stratId} className="bg-gray-900 rounded-lg p-4 border border-gray-800">
                      <div className="flex items-center justify-between mb-3">
                        <span className="text-sm font-medium">{stratId}</span>
                        <span className="text-xs text-gray-400">{strat.signal_count} signals</span>
                      </div>
