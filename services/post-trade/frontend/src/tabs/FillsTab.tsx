  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        <Card title="Avg Slippage (bps)" value={summary.avg_slippage_bps.toFixed(2)} />
              ...
              <tbody>
                {data.fills.map((fill) => (
                  <tr key={fill.fill_id} className="border-b border-gray-800/50 hover:bg-gray-900/50">
                    <td className="py-2 px-3 text-xs text-gray-400">{formatTimestamp(fill.timestamp)}</td>
                    ...
                    <td className="text-right py-2 px-3">{fill.quantity.toFixed(6)}</td>
                    <td className="text-right py-2 px-3">${fill.fill_price.toLocaleString()}</td>
                    <td className="text-right py-2 px-3">${fill.fee.toFixed(4)}</td>
                    <td className="text-right py-2 px-3">{fill.slippage_bps.toFixed(2)}</td>
                    <td className="py-2 px-3 text-xs text-gray-400">{fill.strategy_id}</td>
                  </tr>
                ))}
              </tbody>
            </table>
